...
struct NativeBackendCheck {
    static func main() throws {
        ...
        let session = home.appendingPathComponent("sessions/rollout-selected.jsonl")
        try FileManager.default.createDirectory(at: session.deletingLastPathComponent(), withIntermediateDirectories: true)
        try Data("{\"type\":\"session_meta\",\"payload\":{\"id\":\"selected\",\"model_provider\":\"old\",\"model\":\"gpt-old\"}}\n{\"type\":\"event\"}\n".utf8).write(to: session)
        try Data("keep-index\n".utf8).write(to: paths.sessionIndex)
        let sessionPath = session.path.replacingOccurrences(of: "'", with: "''")
        let sql = "CREATE TABLE threads (id TEXT PRIMARY KEY, title TEXT, cwd TEXT, model_provider TEXT, model TEXT, archived INTEGER, updated_at TEXT, rollout_path TEXT); INSERT INTO threads VALUES ('selected', 'Selected', '/tmp/project', 'old', 'gpt-old', 0, '1', '\(sessionPath)'), ('untouched', 'Untouched', '/tmp/project', 'old', 'gpt-old', 0, '2', ''), ('archived', 'Archived', '/tmp/project', 'old', 'gpt-old', 1, '3', '');"
        ...
        precondition(result.updatedSessionFiles == 1)
        let indexAfterSync = try Data(contentsOf: paths.sessionIndex)
        precondition(String(decoding: indexAfterSync, as: UTF8.self) == "keep-index\n")
