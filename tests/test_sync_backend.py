...
class SyncBackendTests(unittest.TestCase):
    ...
    def test_display_title_collapses_whitespace_and_limits_payload(self) -> None:
        ...
        self.assertTrue(title.startswith("hello world "))
        self.assertEqual(len(title), 240)
        self.assertTrue(title.endswith("…"))

    def test_selected_sync_does_not_touch_unselected_threads(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            codex_home = Path(temp_dir)
            write_config(codex_home)
            create_threads_db(codex_home)

            result = sync_selected(str(codex_home), ["old-provider-old-model"])

            self.assertEqual(result["updated_rows"], 1)
            with closing(sqlite3.connect(codex_home / "state_5.sqlite")) as conn:
                rows = dict(conn.execute("SELECT id, model FROM threads"))
            self.assertEqual(rows["old-provider-old-model"], "gpt-new")
            self.assertEqual(rows["new-provider-old-model"], "gpt-old")

    def test_selections_are_persisted_and_backup_bundles_are_deleted_together(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            codex_home = Path(temp_dir)
            write_config(codex_home)
            create_threads_db(codex_home)
            ...
            save_selections(codex_home, ["a", "b", "a"])
            self.assertEqual(read_selections(codex_home), {"a", "b"})

            backup = make_backup(paths, "manual")
            self.assertEqual(len(backup_items(str(codex_home))["backups"]), 1)
            result = delete_backups(str(codex_home), [backup.name])

            self.assertEqual(result["deleted_backups"], 1)
            self.assertEqual(backup_items(str(codex_home))["backups"], [])
    ...
    def test_sync_ignores_archived_threads(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            codex_home = Path(temp_dir)
            write_config(codex_home)
            with closing(sqlite3.connect(codex_home / "state_5.sqlite")) as conn:
                ...
                conn.executemany(
                    ...
                    [("active", "old", "gpt-old", 0), ("archived", "old", "gpt-old", 1)],
                )
                conn.commit()

            paths = resolve_paths(str(codex_home))
            self.assertEqual(get_status(paths)["movable_threads"], 1)
            self.assertEqual(sync_to_current_provider(paths)["updated_rows"], 1)
            with closing(sqlite3.connect(codex_home / "state_5.sqlite")) as conn:
                archived = conn.execute(
                    "SELECT model_provider, model FROM threads WHERE id = 'archived'"
                ).fetchone()
            self.assertEqual(archived, ("old", "gpt-old"))
