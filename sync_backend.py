...
def sync_session_records(
    ...
    for record in before_records:
        if active_thread_ids is not None and record.thread_id not in active_thread_ids:
            continue
        ...
        try:
            with connect_db(
                ...
                if "archived" in columns:
                    where_sql = f"archived = 0 AND ({where_sql})"
...
def get_status(paths: Paths) -> dict[str, object]:
    ...
    with connect_db(paths.db_path, readonly=True) as conn:
        columns = get_thread_columns(conn)
        active_condition = "archived = 0 AND " if "archived" in columns else ""
        ...
        provider_movable = int(conn.execute(
            f"SELECT COUNT(*) FROM threads WHERE {active_condition}(model_provider IS NULL OR model_provider <> ?)",
            (current_provider,),
        ).fetchone()[0])
        model_movable = int(conn.execute(
            f"SELECT COUNT(*) FROM threads WHERE {active_condition}(model IS NULL OR model <> ?)",
            (current_model,),
        ).fetchone()[0]) if "model" in columns and current_model else None
        where_parts = ["model_provider IS NULL OR model_provider <> ?"]
        ...
        where_sql = " OR ".join(f"({part})" for part in where_parts)
        db_movable_ids = {str(row["id"]) for row in conn.execute(
            f"SELECT id FROM threads WHERE {active_condition}({where_sql})", params
        )}
        ...
        session_movable_ids &= db_thread_ids
...
def make_backup(paths: Paths, label: str) -> Path:
    ...
    # 使用 SQLite backup API，而不是复制正在写入的数据库文件。
...
def sync_to_current_provider(paths: Paths) -> dict[str, object]:
    ...
    with connect_db(paths.db_path, readonly=True) as conn:
        columns = get_thread_columns(conn)
        where_sql = "WHERE archived = 0" if "archived" in columns else ""
        active_thread_ids = {str(row["id"]) for row in conn.execute(f"SELECT id FROM threads {where_sql}")}
    session_summary = sync_session_records(paths, current_provider, current_model, active_thread_ids)
